  ...
  village_interfaces__msg__Task__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    village_interfaces__msg__Task * data =
      (village_interfaces__msg__Task *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!village_interfaces__msg__Task__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          village_interfaces__msg__Task__fini(&data[i]);
        }
        free(data);
        return false;
      }
